...
namespace Mozlite.Core.Tasks
{
    ...
    public class TaskInterval
    {
        ...
        public static implicit operator TaskInterval(string date)
        {
            ...
            var interval = date.AsInt32();
            if (interval != null)
                return new TaskInterval(interval.Value);
            ...
            if (dates.Length == 1)
                return new TaskInterval(day: dates[0].AsInt32() ?? 0, time: TimeSpan.Parse(dateTimes[1]));
            return new TaskInterval(month: dates[dates.Length - 2].AsInt32() ?? 0, day: dates[dates.Length - 1].AsInt32() ?? 0, time: TimeSpan.Parse(dateTimes[1]));
        }
