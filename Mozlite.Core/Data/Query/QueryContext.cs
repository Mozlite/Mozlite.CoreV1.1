...
namespace Mozlite.Data.Query
{
    ...
    public class QueryContext<TModel> : IQueryable<TModel>, IQuerySql
    {
        ...
        /// <returns>返回当前查询实例对象。</returns>
        IQueryContext<TModel> IQueryContext<TModel>.Select(Ignore ignore)
            => Select(ignore);
        ...
        /// <param name="ignore">忽略属性。</param>
        /// <returns>返回当前查询实例对象。</returns>
        public IQueryable<TModel> Select(Ignore ignore)
        {
            if (ignore == Ignore.None)
                return Select();
            _fields.AddRange(Entity.FindProperties(ignore).Select(p => Delimit(p.Name)));
            return this;
        }
