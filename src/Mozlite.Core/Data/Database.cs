...
namespace Mozlite.Data
{
    ...
    public abstract class Database : IDatabase
    {
        ...
        public virtual bool BeginTransaction(Func<ITransaction, bool> executor, int timeout = 30)
        {
            using (var connection = GetConnection())
            {
                connection.Open();
        ...
        /// <returns>返回事务实例对象。</returns>
        public virtual async Task<bool> BeginTransactionAsync(Func<ITransaction, Task<bool>> executor, int timeout = 30, CancellationToken cancellationToken = new CancellationToken())
        {
            using (var connection = GetConnection())
            {
