...
namespace Mozlite.Mvc.TagHelpers.Binders
{
    ...
    public class DocumentElement : Element
    {
        ...
        private bool ReadCodeBlock(Element parent)
        {
            ...
            var current = NewBlock(code);
            parent.Add(current);
            Read(current);
            return false;
        }

        private CodeBlockElement NewBlock(string code)
        {
            var index = code.IndexOf(' ');
            if (index == -1)
                return new CodeBlockElement(code, null);
            return new CodeBlockElement(code.Substring(0, index), code.Substring(index).Trim(' ', '(', ')'));
        }
